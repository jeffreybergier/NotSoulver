...
{
  return [[[TinyRegex alloc] initWithString:string pattern:pattern] autorelease];
}

+(id)regexWithString:(NSString*)string pattern:(NSString*)pattern forceIteration:(BOOL)forceIteration;
{
  return [[[TinyRegex alloc] initWithString:string pattern:pattern forceIteration:forceIteration] autorelease];
}
...
@implementation TinyRegex (Tests)
...
+(void)__executeTests_ranges;
{
  ...
  regex = [TinyRegex regexWithString:@"this is a verb for other cool verbs" pattern:@"verb"];
  NSAssert(regex, @"");
  ...
  regex = [TinyRegex regexWithString:@"12.34a5678m-90.12s-3456" pattern:@"-?\\d+\\.?\\d+"];
  NSAssert([regex containsMatch], @"");
  range = [regex nextMatch];
  ...
  regex = [TinyRegex regexWithString:@"12.34a5678m999m-90.12s-3456" pattern:@"-?\\d+\\.?\\d+[ma]-?\\d+\\.?\\d+"];
  NSAssert([regex containsMatch], @"");
  range = [regex nextMatch];
  NSAssert(range.location == 0, @"");
  ...
  regex = [TinyRegex regexWithString:@"12.m56...78m--90.12" pattern:@"-?\\d+\\.?\\d+m-?\\d+\\.?\\d+"];
  NSAssert(![regex containsMatch], @"");
  
  // MARK: Test Multiple Operators
  regex = [TinyRegex regexWithString:@"5+7+3" pattern:@"[\\d\\)][\\*\\-\\+\\/\\^][\\-\\d\\(]" forceIteration:YES];
  NSAssert([regex containsMatch], @"");
  range = [regex nextMatch];
  NSAssert(range.location == 0, @"");
  ...
  regex = [TinyRegex regexWithString:@"3*5^2+7" pattern:@"\\d\\^\\d"];
  NSAssert([regex containsMatch], @"");
  range = [regex nextMatch];
  NSAssert(range.location == 2, @"");
  NSAssert(range.length == 3, @"");
  ...
  NSAssert(range.location == NSNotFound, @"");
  NSAssert(range.length == 0, @"");
  regex = [TinyRegex regexWithString:@"3*5^2" pattern:@"\\d[\\^\\*]\\d" forceIteration:YES];
  NSAssert([regex containsMatch], @"");
  range = [regex nextMatch];
  NSAssert(range.location == 0, @"");
  NSAssert(range.length == 3, @"");
